...
def test_order_reservation_system(food_commodity, mock_sim) -> None:
    ...
    assert (
        seller.inventory.get_available_quantity(food_commodity) == 5
    )  # Available reduced
    assert (
        seller.inventory.get_reserved_quantity(food_commodity) == 5
    )  # Reserved increased
    assert order_id is not None and order_id != ""
    ...
    assert (
        seller.inventory.get_reserved_quantity(food_commodity) == 0
    )  # All reserved was sold
