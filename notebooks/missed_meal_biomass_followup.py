"""Follow-up: biomass path availability for solvent missers.
        ...
        ROWS.append(
            {
                ...
                "bio_ask": int(bio_ask) if bio_ask is not None else -1,
...
def followup() -> None:
    """Solvent missers only: biomass availability vs the food ask."""
    solvent = [r for r in ROWS if r["best_ask"] >= 0 and r["money"] >= r["best_ask"]]
    print(f"\n[F0] solvent miss rows: {len(solvent)} of {len(ROWS)}")
    splits = [
        ("lo<0.4", [r for r in solvent if r["bucket"] == "lo<0.4"]),
        ("mid+hi", [r for r in solvent if r["bucket"] != "lo<0.4"]),
    ]

    print("\n[F1a] biomass ask present, and make-food cost vs food ask (medians)")
    print(
        f"{'split':<8}{'rows':>7}{'bio_ask%':>10}{'bio_ask':>9}{'mk/unit':>9}"
        f"{'food_ask':>10}{'cheaper%':>10}"
    )
    for name, rows in splits:
        ...
        print(
            ...
            f"{(statistics.median(mk) if mk else -1):>9.1f}"
            f"{statistics.median([r['best_ask'] for r in rows]):>10.0f}"
            f"{pct(cheaper, len(with_ask)):>10}"
        )

    print("\n[F1b] can the misser afford the make-food batch (4*bio_ask)?")
    print(f"{'split':<8}{'rows':>7}{'afford_batch':>14}{'median money':>14}")
    for name, rows in splits:
        with_ask = [r for r in rows if r["bio_ask"] >= 0]
        if not with_ask:
            continue
        aff = sum(1 for r in with_ask if r["money"] >= 4 * r["bio_ask"])
        print(
            f"{name:<8}{len(with_ask):>7}{pct(aff, len(with_ask)):>14}"
            f"{statistics.median([r['money'] for r in with_ask]):>14.0f}"
        )

    print("\n[F2] biomass held at the miss (share of rows)")
    print(f"{'split':<8}{'rows':>7}{'bio=0':>8}{'1-3':>8}{'>=4':>8}{'med':>6}")
    for name, rows in splits:
        if not rows:
            continue
        z = sum(1 for r in rows if r["biomass"] == 0)
        lo = sum(1 for r in rows if 1 <= r["biomass"] <= 3)
        hi = sum(1 for r in rows if r["biomass"] >= 4)
        print(
            f"{name:<8}{len(rows):>7}{pct(z, len(rows)):>8}{pct(lo, len(rows)):>8}"
            f"{pct(hi, len(rows)):>8}"
            f"{statistics.median([r['biomass'] for r in rows]):>6.0f}"
        )
    ...
    def group(action: str) -> str:
        if action == "make_food":
            return "make_food"
        if action == "gather_biomass":
            return "gather_biomass"
        if action == "GovernmentWorkCommand":
            return "gov_work"
        if action == "idle":
            return "none/idle"
        return "other_recipe_or_cmd"
    ...
    print(f"{'split':<8}" + "".join(f"{lbl:>21}" for lbl in labels))
    for name, rows in splits:
        if not rows:
            continue
