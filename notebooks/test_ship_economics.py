...
run_path = os.getenv(
    "SPACESIM_RUN_PATH", "data/runs/ship_economics_run/run_20251128_142009"
)
...
# Filter for ships
ship_actors = all_actors.filter(pl.col("actor_name").str.contains("(?i)ship|trader"))
num_ships = (
    ship_actors.select("actor_name").unique().shape[0] if len(ship_actors) > 0 else 0
)
...
if num_ships > 0:
    ...
    ship_profits = (
        ...
        .with_columns(
            [
                (pl.col("money") - pl.col("money").shift(1).over("actor_name")).alias(
                    "profit_per_turn"
                ),
    ...
    inventory_expanded = ship_actors.with_columns(
        [
            ...
            .map_elements(lambda x: json.loads(x) if x else {}, return_dtype=pl.Object)
