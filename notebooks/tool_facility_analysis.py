...
def _(data, mo):
    if data is None:
        ...
        _total_turns = data.actor_turns['turn'].max() if len(data.actor_turns) > 0 else 0
...
def _(data, mo, pl, px):
    ...
    if data is None or len(data.market_transactions) == 0:
        ...
        if len(_tool_txns) == 0:
            _output = mo.md("**No transactions found for tool/facility commodities.** This may indicate the bootstrap path is not working - actors are not producing or trading these items.")
        else:
            # Aggregate by turn and commodity
            _volume_by_turn = _tool_txns.group_by(['turn', 'commodity_id']).agg(
            ...
            _output = px.line(
                ...
                labels={'volume': 'Units Traded', 'turn': 'Turn', 'commodity_id': 'Commodity'}
            )
    ...
    if data is None or len(data.market_transactions) == 0:
        ...
        if len(_tool_txns) == 0:
            ...
            **No simple_tools transactions found.**
            ...
            _tools_per_turn = _tool_txns.group_by('turn').agg([
            ...
            _fig.add_trace(
                go.Scatter(x=_tools_pd['turn'], y=_tools_pd['cumulative_volume'],
    ...
    if data is None or len(data.actor_turns) == 0:
        ...
        def _parse_inventory(inv_json: str) -> dict:
            ...
            try:
                ...
                return {'simple_tools': 0, 'smelting_facility': 0, 'metalworking_facility': 0,
                       'common_metal': 0, 'common_metal_ore': 0}
        ...
        _agg_inv = _inv_df.group_by('turn').agg([
        ...
        _melted = _agg_inv.unpivot(
            index='turn',
            on=['total_tools', 'total_smelting', 'total_metalworking', 'total_metal', 'total_ore'],
...
def _(data, mo, pl):
    if data is None or len(data.market_transactions) == 0:
        ...
        _bootstrap_df = pl.DataFrame(_first_appearances).sort('first_traded_turn', nulls_last=True)
...
def _(data, mo, pl, px):
    if data is None or len(data.market_transactions) == 0:
        ...
        _volume_by_commodity = _volume_by_commodity.with_columns(
            pl.col('commodity_id').replace(OUTPUT_TO_PROCESS, default='unknown').alias('likely_process')
        )
...
def _(data, mo, pl):
    if data is None:
        ...
        _tool_txns = data.market_transactions.filter(pl.col('commodity_id') == 'simple_tools')
        ...
        _clothing_txns = data.market_transactions.filter(pl.col('commodity_id') == 'clothing')
        _has_clothing = len(_clothing_txns) > 0

        _metal_txns = data.market_transactions.filter(pl.col('commodity_id') == 'common_metal')
        ...
        if _has_tools:
            _findings.append(f"- **Tools are being produced**: {_tools_volume} simple_tools traded")
        else:
            _findings.append("- **ISSUE: No tools being traded** - metalworking chain may be broken")

        if _has_metal:
            _findings.append("- **Metal refining working**: common_metal is being traded")
        else:
            _findings.append("- **ISSUE: No metal being traded** - smelting chain may be broken")

        if _has_wood:
            _findings.append("- **Tool-requiring processes working**: wood is being harvested")
        else:
            _findings.append("- **Note**: No wood being traded - harvest_wood requires tools")

        if _has_clothing:
            _findings.append("- **Clothing production working**: clothing is being traded")
        else:
            _findings.append("- **Note**: No clothing being traded - make_clothing requires tools")
