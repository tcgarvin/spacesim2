...
def render_mermaid_to_temp(mermaid_code, format="svg", open_after=False):
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        try:
            ...
            print(
                "Error: `mmdc` not found. Please install it with: npm install -g @mermaid-js/mermaid-cli"
            )
            return
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Convert YAML to Mermaid and render to image."
    )
    parser.add_argument(
        "--out", "-o", help="Save output to this base filename (no extension)"
    )
    parser.add_argument(
        "--format", "-f", help="Output format (png, svg, pdf)", default="svg"
    )
    parser.add_argument(
        "--open", action="store_true", help="Open the output diagram after rendering"
    )
