...
class UIRenderer:
    ...
    def draw_panel(
        ...
        pygame.draw.rect(
            self.screen, color, rect, border_width, border_radius=border_radius
        )

    def draw_button(
        self, rect: Rect, is_hover: bool = False, border_radius: int = 3
    ) -> None:
        ...
        color = (
            ...
            if is_hover
            else self.colors["ui_elements"]["button"]
        )

        pygame.draw.rect(self.screen, color, rect, border_radius=border_radius)

    def draw_divider(
        self, start_pos: Tuple[int, int], end_pos: Tuple[int, int], width: int = 1
    ) -> None:
        """Draw a divider line.
        ...
        """
        pygame.draw.line(
            self.screen, self.colors["pane_divider"], start_pos, end_pos, width
        )
        ...
        if active_pane in self.colors["active_pane"]:
            if active_pane == 0:  # Left pane
                ...
                pygame.draw.rect(
                    ...
                    (
                        pane_dimensions["left_width"],
                        0,
                        pane_dimensions["center_width"],
                        pane_dimensions["height"],
                    ),
                )
                ...
                pygame.draw.rect(
                    ...
                    (
                        pane_dimensions["left_width"] + pane_dimensions["center_width"],
                        0,
                        pane_dimensions["right_width"],
                        pane_dimensions["height"],
                    ),
                )
