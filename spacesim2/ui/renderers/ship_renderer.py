...
class ShipRenderer:
    ...
    def __init__(
        ...
        screen: Surface,
        colors: Dict,
        planet_positions: Dict[Planet, Tuple[int, int]],
    ) -> None:
    ...
    def draw_ships_in_orbit(
        self, planet: Planet, ships: List[Ship], selected_ship: Optional[Ship] = None
    ) -> None:
        ...
        for i, ship in enumerate(ships_in_orbit):
            ...
            ship_color = (
                self.colors["ship"]["selected"]
                if ship == selected_ship
                else self.colors["ship"]["default"]
            )
            self.draw_ship(ship_x, ship_y, ship_color)

    def draw_ships_in_transit(
        self, ships: List[Ship], selected_ship: Optional[Ship] = None
    ) -> None:
        ...
        for ship in ships_in_transit:
            ...
            pygame.draw.line(
                self.screen, self.colors["ship"]["in_transit"], origin_pos, dest_pos, 1
            )
