...
class PlanetViewPanel:
    ...
    def set_planets(self, planets: List[Planet]) -> None:
        ...
        self.ship_renderer = ShipRenderer(
            self.screen, self.colors, self.planet_renderer.planet_positions
        )
    ...
    def handle_click(self, x: int, y: int) -> bool:
        ...
        for planet in self.planets:
            if planet in self.planet_renderer.planet_positions:
                ...
                name_rect.center = (
                    planet_x,
                    planet_y + self.planet_renderer.planet_radius + 15,
                )
                name_rect.inflate_ip(
                    10, 6
    ...
    def handle_key(self, key: int) -> bool:
        ...
        current_index = (
            self.planets.index(self.selected_planet)
            if self.selected_planet in self.planets
            else 0
        )
    ...
    def render(self, text_renderer: TextRenderer, simulation_ships: List[Ship]) -> None:
        ...
        if self.ship_renderer:
            self.ship_renderer.update_planet_positions(
                self.planet_renderer.planet_positions
            )

            # Draw ships in transit before drawing planets
            self.ship_renderer.draw_ships_in_transit(
                simulation_ships, self.selected_ship
            )
        ...
        for planet in self.planets:
            ...
            if planet in self.planet_renderer.planet_positions:
                ...
                name_surface, name_rect = text_renderer.render_text(
                    ...
                    "small",
                    text_color,
                    center_pos=(pos_x, pos_y + self.planet_renderer.planet_radius + 15),
                )
                ...
                if planet.ships:
                    ship_count = len(
                        [s for s in planet.ships if s.status == ShipStatus.DOCKED]
                    )
                    if ship_count > 0:
                        ship_count_text, ship_count_rect = text_renderer.render_text(
                            ...
                            center_pos=(
                                pos_x,
                                pos_y + self.planet_renderer.planet_radius + 30,
                            ),
                        )
                ...
                if self.ship_renderer and planet.ships:
                    self.ship_renderer.draw_ships_in_orbit(
                        planet, planet.ships, self.selected_ship
                    )
