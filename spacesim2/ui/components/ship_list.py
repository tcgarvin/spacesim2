...
class ShipListPanel:
    ...
    def render(self, text_renderer: TextRenderer) -> None:
        ...
        start_index = (
            min(self.scroll_offset, len(self.selected_planet.ships) - 1)
            if self.selected_planet.ships
            else 0
        )
        end_index = (
            min(start_index + self.visible_items, len(self.selected_planet.ships))
            if self.selected_planet.ships
            else 0
        )
        ...
        for i in range(start_index, end_index):
            ...
            cargo_text, cargo_rect = text_renderer.render_text(
                f"Food: {cargo_food}, Fuel: {cargo_fuel}",
                "small",
        ...
        if start_index > 0:
            ...
            pygame.draw.polygon(
                ...
                [
                    (self.width - 30, header_y + 20),
                    (self.width - 20, header_y + 5),
                    (self.width - 10, header_y + 20),
                ],
            )
        ...
        if end_index < len(self.selected_planet.ships):
            ...
            pygame.draw.polygon(
                ...
                self.colors["text"]["normal"],
                [
                    (self.width - 30, bottom_y - 20),
                    (self.width - 20, bottom_y - 5),
                    (self.width - 10, bottom_y - 20),
                ],
            )
