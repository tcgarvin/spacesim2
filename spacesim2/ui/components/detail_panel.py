...
class DetailPanel:
    ...
    def __init__(
        self, screen: Surface, colors: Dict, x: int, y: int, width: int, height: int
    ) -> None:
    ...
    def handle_click(self, x: int, y: int) -> bool:
        ...
        if self.selected_planet:
            ...
            panel_rect = Rect(self.x + 20, 80, self.width - 40, self.height - 150)
            ...
            if 0 <= x_relative <= panel_rect.width:
                ...
                if y_relative >= commodity_start_y:
                    ...
                    commodity_registry = (
                        self.simulation.commodity_registry if self.simulation else None
                    )
                    if commodity_registry:
                        ...
                        if 0 <= commodity_index < len(commodity_list):
                            ...
                            if (
                                ...
                                == commodity_list[commodity_index]
                            ):
                                self.selected_commodity = None
                            else:
                                self.selected_commodity = commodity_list[
                                    commodity_index
                                ]
                            return True
    ...
    def _render_ship_details(
        self, panel_rect: Rect, text_renderer: TextRenderer
    ) -> None:
        ...
        if commodity_registry:
            for commodity in commodity_registry.all_commodities():
                ...
                price_text, price_rect = text_renderer.render_text(
                    f"• {commodity.name}: ${avg_price:.2f}{bid_ask_str}",
                    "normal",
                    text_color,
                )
    ...
    def _render_commodity_orders(
        self, panel_rect: Rect, x: int, y: int, text_renderer: TextRenderer
    ) -> None:
        ...
        current_transactions = [
            ...
            if tx.commodity_type == commodity
            and tx.turn == self.simulation.current_turn
        ]
        ...
        y += (
            max(len(visible_buy_orders), len(visible_sell_orders)) * line_height
            + line_height
        )
    ...
    def _render_simulation_details(
        self, panel_rect: Rect, text_renderer: TextRenderer
    ) -> None:
        ...
        turn_text, turn_rect = text_renderer.render_text(
            f"Current Turn: {simulation.current_turn}",
            "normal",
            self.colors["text"]["normal"],
        )
        ...
        planet_count_text, planet_count_rect = text_renderer.render_text(
            f"Planets: {len(simulation.planets)}",
            "normal",
            self.colors["text"]["normal"],
        )
        ...
        # Ships in transit
        transit_count = sum(
            1 for ship in simulation.ships if ship.status == ShipStatus.TRAVELING
        )
        if transit_count > 0:
