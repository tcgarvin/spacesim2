...
class ActorListPanel:
    ...
    def __init__(
        self, screen: Surface, fonts: Dict, colors: Dict, width: int, height: int
    ) -> None:
        ...
        self.columns = max(
            1, (width - 2 * self.grid_padding) // (self.actor_size + self.padding)
        )
    ...
    def handle_mouse_motion(self, x: int, y: int) -> bool:
        ...
        if (
            x < self.grid_padding
            or x > self.width - self.grid_padding
            or y < self.grid_start_y
            or y > self.detail_area_start - self.grid_padding
        ):
            self.hovered_actor = None
            return False
    ...
    def handle_key(self, key: int) -> bool:
        ...
        if key == pygame.K_UP:
            if current_index >= 0:
                ...
                # Calculate how many rows are visible
                grid_height = (
                    self.detail_area_start - self.grid_start_y - self.grid_padding
                )
                visible_rows = max(1, grid_height // (self.actor_size + self.padding))
            ...
            if current_index >= 0:
                ...
                if new_index < total_entities:
                    ...
                    visible_rows = max(
                        1, grid_height // (self.actor_size + self.padding)
                    )
    ...
    def render(self, text_renderer: TextRenderer) -> None:
        ...
        if self.selected_planet:
            ...
            actor_count = (
                len(self.selected_planet.actors) if self.selected_planet else 0
            )
        ...
        if not self.selected_planet or (
            not self.selected_planet.actors and not self.selected_planet.ships
        ):
        ...
        for row in range(start_row, end_row):
            for col in range(self.columns):
                ...
                if entity_index < total_entities:
                    ...
                    y = self.grid_start_y + (row - start_row) * (
                        self.actor_size + self.padding
                    )
    ...
    def _render_actor_square(
        self, actor: Actor, x: int, y: int, text_renderer: TextRenderer
    ) -> None:
        ...
        if actor.actor_type == ActorType.MARKET_MAKER:
            ...
            pygame.draw.polygon(self.screen, base_color, diamond_points)
        ...
        if self.context == "market" and self.selected_commodity:
            ...
            if quantity > 0:
                ...
                count_text, count_rect = text_renderer.render_text(
                    str(quantity),
                    "small",
                    (0, 0, 0),  # Black text for visibility
                )
    ...
    def _draw_actor_status_indicators(self, actor: Actor, x: int, y: int) -> None:
        ...
        food_color = (
            self.colors["text"]["food"] if food_status else (150, 50, 50)
        )  # Green if fed, red if hungry
        pygame.draw.circle(self.screen, food_color, (x + 5, y + 5), 3)

        # Top-right: Market activity (e.g., has active orders)
        has_orders = len(actor.active_orders) > 0
        order_color = (
            (50, 200, 50) if has_orders else (100, 100, 100)
        )  # Green if has orders
        pygame.draw.circle(
            self.screen, order_color, (x + self.actor_size - 5, y + 5), 3
        )
        ...
        pygame.draw.circle(
            self.screen, money_color, (x + 5, y + self.actor_size - 5), 3
        )
    ...
    def _render_ship_circle(
        self, ship: Ship, x: int, y: int, text_renderer: TextRenderer
    ) -> None:
        ...
        pygame.draw.circle(self.screen, base_color, (center_x, center_y), radius)
    ...
    def _render_detail_area(self, text_renderer: TextRenderer) -> None:
        ...
        display_entity = (
            self.hovered_actor
            if self.hovered_actor
            else (self.selected_actor if self.selected_actor else self.selected_ship)
        )

        if not display_entity:
            # No entity to display, show a message
            info_text, info_rect = text_renderer.render_text(
                "Hover or select an entity to see details",
                "normal",
                self.colors["text"]["normal"],
            )
            info_rect.center = (
                self.width // 2,
                self.detail_area_start + self.detail_area_height // 2,
            )
        ...
        if isinstance(display_entity, Ship):
            ...
            name_text, name_rect = text_renderer.render_text(
                f"{display_entity.name} (Ship - {display_entity.status.value})",
                "normal",
                status_color,
            )
            ...
            # Actor name and type
            name_color = (
                self.colors["text"]["market_maker"]
                if display_entity.actor_type == ActorType.MARKET_MAKER
                else self.colors["text"]["header"]
            )
            name_text, name_rect = text_renderer.render_text(
                f"{display_entity.name} ({display_entity.actor_type.value})",
                "normal",
                name_color,
            )
            name_rect.topleft = (x, y)
            ...
            food_qty = (
                display_entity.inventory.get_quantity(raw_food) if raw_food else 0
            )
            if food_qty > 0:
        ...
        if not isinstance(display_entity, Ship) and display_entity.skills:
            ...
            significant_skills = {
                skill_id: rating
                for skill_id, rating in display_entity.skills.items()
            ...
            if significant_skills:
                ...
                for skill_id, rating in significant_skills.items():
                    ...
                    percentage_str = (
                        f"+{percent_diff}%" if percent_diff > 0 else f"{percent_diff}%"
                    )
        ...
        has_items = False
        inventory = (
            display_entity.cargo
            if isinstance(display_entity, Ship)
            else display_entity.inventory
        )
        ...
        if (
            not isinstance(display_entity, Ship)
            and hasattr(display_entity, "last_market_action")
            and display_entity.last_market_action
        ):
