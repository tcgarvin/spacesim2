...
class MarketMakerBrain(ActorBrain):
    ...
    def decide_market_actions(self, actor: "Actor") -> List[MarketCommand]:
        ...
        for commodity_type in [
            ...
            fuel_ore_commodity,
            wood_commodity,
            common_metal_commodity,
            common_metal_ore_commodity,
        ]:
            ...
            target_sell_price = ceil(
                norm.ppf(curve_percentile, loc=average_price, scale=price_sigma)
            )
            target_buy_price = floor(
                norm.ppf(curve_percentile, loc=average_price, scale=price_sigma)
            )
            ...
            if market.has_history(commodity_type):
                ...
                if current_inventory > 0:
                    ...
                    sell_percentiles = [
                        curve_percentile + i * curve_percentile_step
                    ...
                    sell_prices = [
                        max(
                            int(norm.ppf(p, loc=average_price, scale=price_sigma)) + 1,
                            2,
                        )
                        for p in sell_percentiles
                    ]
                    ...
                    for price, count in sell_price_counts.items():
                        ...
                        commands.append(
                            PlaceSellOrderCommand(commodity_type, order_size, price)
                        )
                ...
                if allocated_funds > 0:
                    ...
                    buy_prices = [
                        max(int(norm.ppf(p, loc=average_price, scale=price_sigma)), 1)
                        for p in buy_percentiles
                    ]
                    target_order_size = (
                        ceil(allocated_funds / target_buy_price)
                        if target_buy_price > 0
                        else 0
                    )
                    ...
                    for price, count in buy_price_counts.items():
                        if funds_remaining <= 0:
                            break
                        order_size = min(
                            funds_remaining // price, target_order_size * count
                        )
                        if order_size > 0:
                            funds_remaining -= order_size * price
                            commands.append(
                                PlaceBuyOrderCommand(commodity_type, order_size, price)
                            )
            else:
                ...
                for i in [1, 2, 4]:
                    if allocated_funds // i > 0:
                        commands.append(
                            PlaceBuyOrderCommand(
                                commodity_type, 1, allocated_funds // i
                            )
                        )
